...
echo "`npx jq '.scripts += {"amplify:'${PROJECT_NAME}'CognitoPreTokenGeneration": "cd amplify/backend/function/'${PROJECT_NAME}'CognitoPreTokenGeneration/ts && npm install && npm run tsc && cd -"}' package.json`" > package.json
